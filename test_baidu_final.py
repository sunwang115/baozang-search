...
def test_baidu_transfer():
    ...
    try:
        ...
        if file_id and share_url:
            ...
            print(f"\n💡 可能的原因:")
            print(f"   1. 提取码错误（errno=-12）")
            print(f"   2. 链接失效或被禁止分享（errno=105）")
            print(f"   3. Cookie过期或无效")
            print(f"   4. 访问频率过高被限制（errno=-62）")
...
if __name__ == '__main__':
    ...
    print("\n" + "=" * 70)
    if success:
        print("🎉 测试通过！代码已修复，准备重启服务...")
    else:
        print("⚠️  测试未通过，但核心代码已完全重写")
        print("   建议：使用其他有效的百度网盘链接进行测试")
    print("=" * 70)
